...
uibutton_t main_menu_from_game = {
...
void draw_game_over(void) {
    ...
    DrawRectangleV((Vector2){ SCREEN_WIDTH / 4, SCREEN_HEIGHT / 4 },
                   (Vector2){ SCREEN_WIDTH * 0.5, SCREEN_HEIGHT * 0.5 }, ui_button_color);
    ...
    DrawText(high_score_text, SCREEN_WIDTH / 2 - MeasureText(high_score_text, 20) / 2, SCREEN_HEIGHT / 4 + 95, 20,
             RAYWHITE);
    if (new_high_score) {
        DrawText("New High Score!", SCREEN_WIDTH / 2 - MeasureText("New High Score!", 20) / 2, SCREEN_HEIGHT / 4 + 115,
                 20, RAYWHITE);
    }
...
void draw_player_ui(void) {
    ...
    DrawRectangleV((Vector2){ .x = 25, .y = 50 },
                   (Vector2){ .x = Lerp(0, 150, player.meter / player.max_meter), .y = 15 }, BLUE);
    ...
    if (teleport_cooldown_timer.is_running) {
        ...
        DrawRectangleV(
            (Vector2){ .x = 25, .y = 75 },
            (Vector2){ .x = 150 * Lerp(0, 1, (teleport_cooldown_timer.progress / teleport_cooldown_timer.duration)),
                       .y = 8 },
            DARKGRAY);
...
void draw_pause_menu(void) {
    DrawRectangleV((Vector2){ SCREEN_WIDTH / 4, SCREEN_HEIGHT / 3 },
                   (Vector2){ SCREEN_WIDTH * 0.5, SCREEN_HEIGHT * 0.20 }, ui_button_color);
    DrawText("Game Paused!", SCREEN_WIDTH / 2 - MeasureText("Game Paused!", 30) / 2, SCREEN_HEIGHT / 3 + 30, 30,
             RAYWHITE);
}
